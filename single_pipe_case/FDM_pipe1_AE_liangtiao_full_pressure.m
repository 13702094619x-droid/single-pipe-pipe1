...
clear,clc
%% 0 程序说明
% 单管道算例原压差版本：水力部分采用首末压力边界和AE压降方程反算质量流量，温度部分沿用FDM递推。
...
pressure_drop_scale = 1;    %AE原压差算例：首末压差不缩放
...
save(fullfile(script_dir,'FDM_pipe1_AE_liangtiao_full_pressure_temperature.mat'), ...
...
save(fullfile(script_dir,'FDM_pipe1_AE_liangtiao_full_pressure_carbon.mat'), ...
...
if exist(hydraulic_file,'file') ~= 2
    ...
    xlim([0 t_end])
    legend(sprintf('原压力数据库节点%d',pressure_compare_node),sprintf('原压差稳态AE节点%d',pressure_compare_node))
    ylabel('压力/Pa')
    subplot(2,1,2)
